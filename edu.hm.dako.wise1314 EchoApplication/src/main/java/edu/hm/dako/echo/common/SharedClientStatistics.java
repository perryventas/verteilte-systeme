...
public class SharedClientStatistics
{
   ...
   *          RoundTrip Time
   * @param serverTime
   *          Die Zeit, die der Server benötigt hat
   */
  public synchronized void incrReceivedMsgCounter( int i, long rtt,
      long serverTime )
  {
    if ( !inRange( i ) )
      return;
    ...
    clientStatistics[ i ].averageRTT = clientStatistics[ i ].sumRTT
        / clientStatistics[ i ].receivedResponses;
    clientStatistics[ i ].sumServerTime = clientStatistics[ i ].sumServerTime
        + serverTime;
  ...
  public synchronized int getNumberOfLostResponses( int i )
  {
    if ( !inRange( i ) )
      return ( -1 );
    return clientStatistics[ i ].sentRequests
        - getNumberOfReceivedResponses( i );
  }
  ...
  public synchronized void printClientStatistic( int i )
  {
    ...
    System.out
        .println( "********************** Client-Statistik *****************************"
            + "\n" + "Sende-/Empfangsstatistik des Clients mit Id "
            + i
            + " ("
            ...
            + "\n"
            + "Anzahl verlorener Responses: "
            + this.getNumberOfLostResponses( i )
            + "\n"
            + "Durchschnittliche RTT: "
            + this.getAverageRTT( i )
            + " ns = "
            + this.getAverageRTT( i ) / 1000000
            + " ms"
            + "\n"
            + "Gesamte RTT: "
            + this.getSumRTT( i )
            ...
            + ( this.getSumRTT( i ) - this.getSumServerTime( i ) )
            + " ns = "
            + ( this.getSumRTT( i ) - this.getSumServerTime( i ) / 1000000 )
            + " ms"
            + "\n"
  ...
  public synchronized void printStatistic()
  {
    ...
    System.out
        .println( "*********************************************************************"
            ...
            + numberOfClients
            + "\n"
            + "Denkzeit des Clients zwischen zwei Requests: "
            + clientThinkTime
            + " ms"
            + "\n"
            ...
            + "\n"
            + "Anzahl empfangener Responses: "
            + this.getSumOfAllReceivedMessages()
            + " von erwarteten "
            + numberOfAllMessages
            + "\n"
            + "\n"
            + "Gesamte RTT ueber alle Clients: "
            + this.getSumRTT()
            + " ns ("
            + ( this.getSumRTT() / 1000000 )
            + " ms)"
            + "\n"
            + "Gesamte Serverzeit ueber alle Clients: "
            + this.getSumServerTime()
            + " ns  ("
            + ( this.getSumServerTime() / 1000000 )
            + " ms)"
            + "\n"
            + "Reine Kommunikationszeit ueber alle Clients: "
            + ( this.getSumRTT() - this.getSumServerTime() )
            + " ns ("
            + ( ( this.getSumRTT() - this.getSumServerTime() ) / 1000000 )
            + " ms)"
            + "\n\n"
            ...
            + "\n"
            + "Reine Serverzeit: "
            + this.getAverageServerTime() / numberOfClients
            + " ns ("
            + ( this.getAverageServerTime() / numberOfClients ) / 1000000
            + " ms)"
            + "\n"
            + "Maximal erreichte Heap-Belegung: "
            + usedMemoryAsString
            + " MByte"
   ...
   * <p/>
   * Messungstyp als String Implementierungstyp als String Anzahl Client-Threads
   * Durchschnittliche RTT Maximale RTT Minimale RTT Durchschnittliche
   * Serverbearbeitungszeit Anzahl geplanter Requests/Responses Anzahl
   * gesendeter Requests Anzahl empfangener Responses Anzahl verlorener Echo
   * Responses Maximale Heap-Size des Clients Startzeit der Messung Endezeit der
   * Messung
   * <p/>
   * Die Werte fuer die "Maximale Heap-Size des Clients" werden noch nicht
   * ermittelt.
   * <p/>
   * Der Satz wird an das Ende einer bestehenden Datei angehaengt. Die Datei
   * kann zur Testauswertung in Excel weiterverarbeitet werden.
   * 
   * @param fileName
   *          Name der Datei
   * @param implType
  ...
  public synchronized void writeStatisticSet( String fileName, String implType,
      String measureType, String startTime, String endTime )
  {
    ...
    try
    {
      ...
      out.write( measureType + ", " + implType + ", " + numberOfClients + ", "
          + this.getAverageRTT() / 1000000 + ", "
          + this.getMaximumRTT() / 1000000 + ", "
          + this.getMinimumRTT() / 1000000 + ", "
          + this.getAverageServerTime() / 1000000 + ", " + numberOfAllMessages
          + ", " + this.getNumberOfSentRequests() + ", "
          + this.getNumberOfReceivedResponses() + ", "
          + this.getNumberOfLostResponses() + ", " + this.getMaxHeapSize()
          / ( 1024 * 1024 ) + ", " + startTime + ", " + endTime + newLine );

      System.out.println( "Auswertungssatz in Datei " + fileName
          + " geschrieben" );
  ...
  /**
   * Berechnet den tatsaechlich benutzten Heap-Speicher Heap-Groesse in MByte
   */

...
public abstract class AbstractClient implements Runnable
{
   ...
   *          Der zu verwendende Client
   */
  public AbstractClient( int serverPort, String remoteServerAddress,
      int clientNumber, int messageLength, int numberOfMessagesToSend,
      ...
      ConnectionFactory connectionFactory )
  {
    ...
    Thread.currentThread().setName(
        "EchoClient-" + String.valueOf( clientNumber + 1 ) );
  ...
  protected final void postReceive( int i, EchoPDU receivedPdu, long rtt )
  {
    ...
    log.debug( threadName + ": RTT fuer Request " + ( i + 1 ) + ": " + rtt
        + " ns" );
    log.debug( threadName + ": Echo-Nachricht empfangen von  "

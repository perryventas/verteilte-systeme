 ...
 * 
 * $Id: tibjmsUtilities.java 21731 2006-05-01 21:41:34Z $
 * 
 */
...
public class tibjmsUtilities
{
      ...
      String password ) throws NamingException
  {
    ...
    {
      System.out
          .println( "Failed to create JNDI InitialContext with provider URL set to "
  ...
  public static Object lookup( String objectName ) throws NamingException
  {
    ...
    /* check if not initialized, then initialize with default parameters */
    initJNDI( null );
  ...
  /**
   * This method creates a java.Security.SecureRandom object seeded with the
   * current time. It allows the samples that use SSL to initialize the SSL
   * environment much faster than if they had to generate a truly random seed.
   * 
   * NOTE: THIS SHOULD NOT BE USED IN A PRODUCTION ENVIRONMENT AS IT IS NOT
   * SECURE.
   */
  public static SecureRandom createUnsecureRandom() throws JMSSecurityException
  {
  ...
  public static void sslUsage()
  {
    ...
    System.err.println( "" );
    System.err
        .println( " -ssl_vendor               <name>      - SSL vendor: 'j2se' or 'entrust6'" );
    System.err
        .println( " -ssl_trace                            - trace SSL initialization" );
    System.err
        .println( " -ssl_vendor_trace                     - trace SSL handshake and related" );
    System.err
        .println( " -ssl_trusted[n]           <file-name> - file with trusted certificates," );
    System.err
        .println( "                                         this parameter may repeat if more" );
    System.err
        .println( "                                         than one file required" );
    System.err
        .println( " -ssl_verify_hostname                  - do not verify certificate name." );
    System.err
        .println( "                                         (this disabled by default)" );
    System.err
        .println( " -ssl_expected_hostname    <string>    - expected name in the certificate" );
    System.err
        .println( " -ssl_custom                           - use custom verifier (it shows names" );
    System.err
        .println( "                                         always approves them)." );
    System.err
        .println( " -ssl_identity             <file-name> - client identity file" );
    System.err
        .println( " -ssl_issuer[n]            <file-name> - client issuer file" );
    System.err
        .println( " -ssl_private_key          <file-name> - client key file (optional)" );
    System.err
        .println( " -ssl_password             <string>    - password to decrypt client identity" );
    System.err.println( "                                         or key file" );
    System.err
        .println( " -ssl_ciphers              <suite-name(s)> - cipher suite names, colon separated" );
    System.exit( 0 );
  }

  private static class SSLParams implements
      com.tibco.tibjms.TibjmsSSLHostNameVerifier
  {
    ...
    public SSLParams( String[] args )
    {
      ...
      while ( i < args.length )
      {
        ...
        else if ( args[ i ].compareTo( "-ssl_trace" ) == 0 )
        {
          ssl_trace = true;
          i += 1;
        }
        else if ( args[ i ].compareTo( "-ssl_debug_trace" ) == 0 )
        {
          ssl_debug_trace = true;
          i += 1;
        }
        else if ( args[ i ].compareTo( "-ssl_expected_hostname" ) == 0 )
        {
          if ( ( i + 1 ) >= args.length )
            sslUsage();
        ...
        else if ( args[ i ].compareTo( "-ssl_verify_hostname" ) == 0 )
        {
          ssl_verify_hostname = true;
          i += 1;
        }
        else if ( args[ i ].compareTo( "-ssl_custom" ) == 0 )
        {
          ssl_custom = true;
          i += 1;
        }
        else if ( args[ i ].compareTo( "-ssl_ciphers" ) == 0 )
        {
          if ( ( i + 1 ) >= args.length )
            sslUsage();
        ...
        else if ( args[ i ].compareTo( "-ssl_private_key" ) == 0 )
        {
          if ( ( i + 1 ) >= args.length )
            sslUsage();
        ...
        else if ( args[ i ].compareTo( "-ssl_trusted" + trusted_suffix ) == 0 )
        {
          if ( ( i + 1 ) >= args.length )
            sslUsage();
        ...
        else if ( args[ i ].compareTo( "-ssl_issuer" + issuer_suffix ) == 0 )
        {
          if ( ( i + 1 ) >= args.length )
            sslUsage();
          String cert = args[ i + 1 ];
    ...
    public void init() throws JMSSecurityException
    {
      ...
      if ( ssl_identity != null )
        com.tibco.tibjms.TibjmsSSL.setIdentity( ssl_identity, ssl_private_key,
    ...
    public void verifyHostName( String connectedHostName,
        String expectedHostName, String certificateCN,
        java.security.cert.X509Certificate server_certificate )
        throws JMSSecurityException
    {
      System.err.println( "HostNameVerifier: " + "    connected = ["

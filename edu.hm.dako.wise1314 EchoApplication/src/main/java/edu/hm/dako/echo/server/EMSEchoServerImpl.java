...
public class EMSEchoServerImpl implements EchoServer, MessageListener
{
  ...
  private Boolean connectToEms()
  {
    ...
    while ( !isConnected )
    {
      try
      {
        this.connection = queueFactory.createQueueConnection( userName,
            password );
  ...
  public void start()
  {
    ...
    if ( this.isConnected )
    {
      new EchoWorker( this.connection, this.session, this.receiver,
          this.publisher ).run();
    }
  ...
  private class EchoWorker implements Runnable
  {
    ...
    private EchoWorker( QueueConnection connection, QueueSession session,
        QueueReceiver rec, TopicPublisher pub )
    {
      this.sess = session;
      this.con = connection;
      this.rec = rec;

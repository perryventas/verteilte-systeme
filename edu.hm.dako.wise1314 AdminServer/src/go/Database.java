	...
	public Count getClientCount(int clientId) throws Exception {
		try {
			...
			connect = DriverManager.getConnection("jdbc:mysql://"
					+ CONSTANTS.DB_HOST + "/" + CONSTANTS.DB_NAME + "?user="
					+ CONSTANTS.DB_USER_NAME + "&password="
					+ CONSTANTS.DB_PASSWORD);

			statement = connect.createStatement();
			resultSet = statement
					.executeQuery("select count(count) as COUNTNR, MIN(CTIMESTAMP) as CTMAX,"
							+ " MAX(CTIMESTAMP) as CTMIN FROM "
							+ CONSTANTS.COUNT_TABLE_NAME);
